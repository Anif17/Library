...
if (isset($_POST['submit'])) {
    ...
    window.location.href='index.html' 
        ...
        $sql = $mysqli->query("SELECT * FROM `tb_student` WHERE `stu_id` = '$username' AND 
        `stu_pass` = '$password'");
        if (mysqli_num_rows($sql) > 0) {
            $info = mysqli_fetch_array($sql);     // returns a row from a recordset
            $_SESSION['stud_id'] = $info['stu_id'];    // assign field in username to session [user]
            echo ("<SCRIPT LANGUAGE='JavaScript'> 
    ...
    window.location.href='index.php' 
        ...
        $sql = $mysqli->query("SELECT * FROM `tb_librarian` WHERE `username` = '$username' AND 
            `password` = '$password'");
